...
internal static class ViewmodelEditor
{
    ...
    private static readonly string[] AnimPresets = { "Equip", "Fire", "ReloadStart", "ReloadEnd", "Cock", "Custom" };
    ...
    private static void CaptureBaselineHands()
    {
    ...
    private static void UpdateAnimPlayback()
    {
        ...
        if (!_animClip.TrySample(normalized, out var sample))
        {
            _animPlaying = false;
            return;
        }
    ...
    private static void ApplyAnimSample(ViewmodelAnimKeyframe sample)
    {
    ...
    internal static void DrawGui()
    {
        ...
        var rect = new Rect(Screen.width * 0.5f - PanelWidth * 0.5f, 30f, PanelWidth, PanelHeight);
        GUI.Box(rect, string.Empty, ImGuiSkinHelper.PanelStyle);

        var headerRect = new Rect(rect.x + 12f, rect.y + 8f, rect.width - 24f, 22f);
        GUI.Label(headerRect, $"Viewmodel Editor — {_bindStatus}", ImGuiSkinHelper.LabelStyle);

        var tabRect = new Rect(rect.x + 12f, rect.y + 32f, rect.width - 24f, 24f);
        ...
        var scrollOuter = new Rect(rect.x + 10f, rect.y + 62f, rect.width - 20f, rect.height - 118f);
        var contentHeight = _activeTab == 0 ? 980f : 760f;
    ...
    private static void DrawAvatarHandsFields()
    {
        ...
        GUILayout.Space(6f);
        GUILayout.Label("Avatar hand offsets", ImGuiSkinHelper.LabelStyle);
        _editProfile.LeftHandOffset = ViewmodelEditorFields.Vector3Field("Left position", _editProfile.LeftHandOffset, -0.2f, 0.2f);
        _editProfile.LeftHandEuler = ViewmodelEditorFields.Vector3Field("Left euler", _editProfile.LeftHandEuler, -45f, 45f);
        _editProfile.RightHandOffset = ViewmodelEditorFields.Vector3Field("Right position", _editProfile.RightHandOffset, -0.2f, 0.2f);
        _editProfile.RightHandEuler = ViewmodelEditorFields.Vector3Field("Right euler", _editProfile.RightHandEuler, -45f, 45f);
    }
    ...
    private static void DrawAnimationsTab()
    {
        ...
        GUILayout.BeginHorizontal();
        for (var i = 0; i < AnimPresets.Length; i++)
        {
            if (GUILayout.Toggle(_animPresetIndex == i, AnimPresets[i], ImGuiSkinHelper.ButtonStyle, GUILayout.Height(20f)))
            {
                _animPresetIndex = i;
                _animClip.Name = AnimPresets[i];
            }
        }
        GUILayout.EndHorizontal();
        ...
        _animScrub = ViewmodelEditorFields.Slider("Timeline", _animScrub, 0f, 1f);
        if (!_animPlaying && _animClip.TrySample(_animScrub, out var scrubSample))
        ...
        for (var i = 0; i < _animClip.Keyframes.Count; i++)
        {
            ...
            if (GUILayout.Toggle(selected, $"t={keyframe.Time:0.###} aim={keyframe.Aim:0.##}", ImGuiSkinHelper.ButtonStyle))
            {
                _selectedKeyframe = i;
